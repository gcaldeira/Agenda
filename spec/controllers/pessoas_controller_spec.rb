...
describe PessoasController do
  ...
  def mock_pessoa(stubs={})
    (@mock_pessoa ||= mock_model(Pessoa).as_null_object).tap do |pessoa|
      pessoa.stub(stubs) unless stubs.empty?
    end
  end

  describe "GET index" do
    it "assigns all pessoas as @pessoas" do
      Pessoa.stub(:all) { [mock_pessoa] }
      get :index
      assigns(:pessoas).should eq([mock_pessoa])
    end
  end

  describe "GET show" do
    it "assigns the requested pessoa as @pessoa" do
      Pessoa.stub(:find).with("37") { mock_pessoa }
      get :show, :id => "37"
      assigns(:pessoa).should be(mock_pessoa)
    end
  end

  describe "GET new" do
    it "assigns a new pessoa as @pessoa" do
      Pessoa.stub(:new) { mock_pessoa }
      get :new
      assigns(:pessoa).should be(mock_pessoa)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created pessoa as @pessoa" do
        Pessoa.stub(:new).with({'these' => 'params'}) { mock_pessoa(:save => true) }
        post :create, :pessoa => {'these' => 'params'}
        assigns(:pessoa).should be(mock_pessoa)
      end

      it "redirects to the created pessoa" do
        Pessoa.stub(:new) { mock_pessoa(:save => true) }
        post :create, :pessoa => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved pessoa as @pessoa" do
        Pessoa.stub(:new).with({'these' => 'params'}) { mock_pessoa(:save => false) }
        post :create, :pessoa => {'these' => 'params'}
        assigns(:pessoa).should be(mock_pessoa)
      end

      it "re-renders the 'new' template" do
        Pessoa.stub(:new) { mock_pessoa(:save => false) }
        post :create, :pessoa => {}
  ...
  describe "PUT update" do
    ...
    describe "with invalid params" do
      it "assigns the pessoa as @pessoa" do
        Pessoa.stub(:find) { mock_pessoa(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:pessoa).should be(mock_pessoa)
      end
